...
buyBtn.addEventListener("click", async () => {
  const { data } = await useFetch("https://be-website-sepatu-g4t6.vercel.app/api/cart/checkout", {
    method: "POST",
    headers: {
...
const renderPricing = async () => {
  ...
  const { data } = await useFetch("https://be-website-sepatu-g4t6.vercel.app/api/cart/total", {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      authorization: "Bearer " + localStorage.getItem("token"),
